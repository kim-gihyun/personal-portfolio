...
/**
 * Lightweight drafting cursor: full-screen crosshair guides, a live X/Y readout,
 * a grid-snap marker, and corner brackets that lock onto interactive elements.
 * All DOM-transform based (updates only on mousemove) — no per-frame canvas.
 */
const GRID = 28;
const LOCK_SEL =
  'a, button, [data-cursor], [role="button"], input, summary, .pf-index-item, .journey-place, .song, .gallery-thumb, .map-city, .hub-link';
// anything that isn't blank background — coords hide over these
const CONTENT_SEL =
  "a,button,input,summary,p,h1,h2,h3,h4,h5,h6,li,b,strong,em,i,code,dt,dd,img,svg,figure,figcaption,article,table,blockquote,pre,label,span:not(.xc-h):not(.xc-v):not(.xc-dot):not(.xc-snap):not(.xc-read),.spec-card,.pf-stage,.journey-map,.journey-card,.gear-feature,.gear-line";
